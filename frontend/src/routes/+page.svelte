		...
		try {
			...
			if (result.ok) {
				...
				await Dialogs.Error({
					...
					Message: result.error || "The selected folder is not a valid LevelDB database."
				});
			}
		} catch (err) {
...
{#if dbPath}
	...
	<div class="editor-layout">
		<header class="editor-header">
			...
			<span class="db-path" title={dbPath}>{dbPath.split(/[/\\]/).pop() || dbPath}</span>
		</header>
		...
		<div class="editor-main">
			...
			<section class="value-panel">
				<h3>Value {#if selectedKey}<span class="key-hint">({selectedKey.length > 40 ? selectedKey.slice(0, 40) + "…" : selectedKey})</span>{/if}</h3>
				{#if valueLoading}
...
{:else}
	...
	<div class="startup-container">
		...
		<button class="btn-open" on:click={openDatabaseFromDialog} disabled={loading}>
			...
			<section class="recent-section">
				...
				<ul class="recent-list">
					{#each recentPaths as item}
						<li>
							<button class="recent-item" on:click={() => openDatabaseFromPath(item.path)} disabled={loading}>
								{item.label}
							</button>
						</li>
					{/each}
